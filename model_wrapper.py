...
class ModelInference:
    ...
    def generate_stream(self, input_text, max_new_tokens=4096, top_p=0.9):
        ...
        self.gen_thread = Thread(target=self.model.generate, kwargs=kwargs)
        self.gen_thread.start()

        for token in itertools.chain(
            [f"## Generated by {self.model_name} model\n"], self.streamer
        ):
            token = token.replace("<|endoftext|>", "").replace("<|end_of_text|>", "")
...
def bg_init_all_models():
    ...
    for model_name, model_path in available_models.items():
        pb.progress(
            len(all_models.keys()) * 0.3333,
            text=f"Loading {model_name} {'model' if model_name=='Fine-Tuned' else ''}",
        )
